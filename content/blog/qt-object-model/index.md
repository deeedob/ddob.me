...
{{ codemirror_load(path="src/main.moc", language="cpp", key="moc") }}
{{ codemirror_load(path="src/CMakeLists.txt", language="cmake", key="cmake") }}

Usually, when working with Qt the direct interaction with its very core functionality is not
required that often. When coding we like to abstract things, and with Qt ? - well, if you have
to abstract 10 or more different platforms, you better make sure your abstraction is tight. Systems
grow like trees, and even the most outer leave will have full dependency on it's roots functionality.

In this post I will try to peel the layers back. I will show you my representation of how the most
important things as they are right now.

When creating GUI applications, modularity, flexibility, and reactivity become essential cornerstones of the development workflow. But even long-running, non-graphical applications — such as services or background processes — can benefit from the same properties.

If an application runs for a longer period, it will hopefully not spend hours calculating a Fibonacci sequence, but rather react to something: incoming data, user input, or external signals. In such cases, reactivity and communication between components are key.

The Qt Object Model provides the central mechanism for this communication. `QObject` is the base class that encapsulates and manages these core features. It is so fundamental to the Qt ecosystem that it’s worth examining it more closely.
...
{{ codemirror(key="moc", section="task-moc") }}
